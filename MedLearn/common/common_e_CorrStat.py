...
class CorrStat(ModelBase):
    ...
    def run(self, 
        ...
        if m:
            if method == 'pearson':
                ...
                if (x_numer_cols ==[]) | (y_numer_cols == []):
                    ...
                    if (x_cate_cols !=[]) | (x_cate_cols != []):
                        logging.warning('input DataFrame has no numeric data columns, will be ignored!')
                    ...
                    dfres = pd.DataFrame(res,columns=['x-列','y-列','相关系数','p-值'])
            ...
            logging.error("unknow method, only 'pearson','kendall'、'spearman' are supported!")
            dfres = pd.DataFrame()
